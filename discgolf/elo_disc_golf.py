#!/usr/bin/env python3
"""
Disc Golf Elo Rating Calculator

Reads an Excel file of player scores, computes Elo ratings across rounds, and writes
...
Usage:
  python -m discgolf.elo_disc_golf --input "2025-26 Rankings History Sep 13.xlsx" --sheet "Sheet1"
...
def expected_score(r_i: float, r_j: float, cap: int = 300) -> float:
    ...
    Note: The original snippet capped `diff` but mistakenly computed with (r_j - r_i).
    We fix that here so the capped diff is actually used.
    """
    diff = r_j - r_i
    if diff > cap:
        diff = cap
...
def compute_multiplayer_elo_weighted_with_all_snapshots(
    ...
    """Compute Elo ratings for a multi-player, per-round dataset.
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Disc Golf Elo Rating Calculator")
